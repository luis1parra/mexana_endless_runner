  ...
  return (
    <div className="flex min-h-screen flex-col bg-white text-[#0B1E52]">
      <main className="flex mx-auto w-full flex-1 px-2 py-6 lg:px-10 lg:py-16">
        <section className="relative w-full rounded-[24px] bg-white bg-[url('../assets/images/rankingbackground_mobile.png')] bg-cover bg-center bg-no-repeat px-3 py-12 pt-2 text-white sm:bg-[url('../assets/images/gamedinamicbackground.png')] md:px-8 lg:rounded-[48px] lg:px-12">
          ...
          <div className="relative mt-8">
            <div>
              ...
              <h1 className="mt-2 text-[44px] font-black leading-[1.04] tracking-tight text-white drop-shadow-md md:text-[56px] lg:text-[64px]">
                Clasificación de Jugadores
              </h1>
            </div>

            <div className="flex flex-col lg:flex-row">
              <div className="mt-4 flex-1 flex-col mx-5 lg:flex-3">
                {leftRanking.map((player) => (
                  <div
                    key={player.position}
                    className="flex items-center gap-4 px-5 py-4"
                  >
                    <div className="flex h-14 w-14 items-center justify-center text-2xl font-bold text-white">
                      {player.position}
                    </div>
                    ...
                    <div className="flex flex-1 flex-col">
                      ...
                      <div className="mt-2 h-4 w-full overflow-hidden rounded-full border border-white/40 bg-white/15">
                        <div
              ...
              <div className="mt-4 flex-1 flex-col mx-5 lg:flex-5">
                <div className="relative w-full overflow-hidden rounded-[44px] bg-white/60 px-4 py-2 text-[#11308F] shadow-[0_25px_45px_rgba(15,31,91,0.35)] backdrop-blur-xl lg:px-6">
                  <div className="absolute inset-3 rounded-[38px] bg-white/70 blur-3xl" />
                  <div className="relative z-1">
                    <div className="mt-4 max-h-100 space-y-3 overflow-y-auto pr-2">
                      {rightRanking.map((player) => {
                        const avatarSrc = badgeColors[player.avatar] || badgeColors["H"];
                        return (
                          <div
                            key={`table-${player.position}`}
                            className="grid grid-cols-[18px_50px_minmax(0,1fr)_80px] lg:grid-cols-[50px_80px_minmax(0,1fr)_120px] items-center gap-3 rounded-[28px] bg-white/0 px-4 py-3 text-sm font-semibold text-[#11308F] shadow-[0_12px_24px_rgba(17,48,143,0.22)]"
                          >
                            <div className="text-xl font-bold text-[#004ed8]">
                              {player.position}
                            </div>
                            <div className="relative h-12 w-12 lg:h-15 lg:w-15">
                              <Image
                                src={avatarSrc}
                            ...
                            <div className="truncate text-md text-center font-semibold text-[#004ed8] lg:text-xl">
                              {player.name}
                            </div>
                            <div className="text-right text-lg font-bold text-[#004ed8] lg:text-2xl">
                              {player.score.toLocaleString("es-CO", {
                                minimumFractionDigits: 0,
                              })}
                            </div>
