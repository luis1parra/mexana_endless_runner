...
(() => {
  ...
  let speed = 0.01, jumpVelocity = 0.2, isJumping = false, yVelocity = 0;//speed = 0.02;
  ...
  startBtnEl.onclick = async () => {
    // Esperar a que el preloader termine (o timeout)
  ...
  function generateObstacle() {
    ...
    const fbxClone = (typeof window.GET_ASSET_CLONE === "function") ? window.GET_ASSET_CLONE("obstacleFBX") : null;
    let obs;
    if (fbxClone) {
      obs = fbxClone;
  ...
  function generateCoin() {
    const laneX = lanes[Math.floor(Math.random() * 3)];
    const fbxClone = (typeof window.GET_ASSET_CLONE === "function") ? window.GET_ASSET_CLONE("coinFBX") : null;
    let coin;
    if (fbxClone) {
      coin = fbxClone;
