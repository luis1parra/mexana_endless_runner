...
(async () => {
    ...
    function applyCommonTransforms(obj, kind /* 'coin' | 'obstacle' */) {
        obj.traverse((o) => {
            if (o.isMesh) {
                o.castShadow = true;
    ...
    function loadGLB(url, kind) {
        return new Promise((resolve) => {
            gltfLoader.load(
                ...
                (e) => { c
                    onsole.error(`${kind} GLB load error:`, url, e); 
                    resolve(null); 
                }
            );
        });
    ...
    // Flujo principal
    (async () => {
        const [coinModels, obstacleModels, cityModels] = await Promise.all([loadAll(COIN_FBX_URLS, "coin"), loadAll(OBSTACLE_FBX_URLS, "obstacle"), loadAll(CITY_FBX_URLS, "city")]);
